...
module.exports = {
	...
	aliasHandleApiGateway(currentTemplate, aliasStackTemplates) {
		...
		if (exposeApi) {
			...
			// We use a custom "pair" iteratee below, It accepts shorthands in the form
			// [ [ property, value ] ] that will operate on the values of a pair array
			// generated with _.toPairs()
			//
			// FIXME: If this code is run multiple times (which is currently not the case) we'll
			// end up with wrapping our iteratee multiple times.
			_.iteratee = _.wrap(_.iteratee, function(iteratee, func) {
				if (_.isArray(func) && func.length === 1 && _.isArray(func[0]) && func[0].length === 2) {
					return function(pair) {
						return _.has(pair[1], func[0][0]) && pair[1][func[0][0]] === func[0][1];
					};
				}

				return iteratee(func);
			});
			...
			if (!_.isEmpty(apiLambdaPermissions)) {
				...
				_.forOwn(apiLambdaPermissions, (permission, name) => {
					...
					permission.Properties.FunctionName = { Ref: versionName };
					...
					permission.DependsOn = [ versionName ];
